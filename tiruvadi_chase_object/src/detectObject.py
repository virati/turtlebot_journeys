...
#Vineet detectObject based on provided script from Sean
...
#Set to track a BLUE ball
lower= np.array([85,150,80],np.uint8) # Array (H,S,V) for the lower threshold bound of the HSV image
upper= np.array([130,255,190],np.uint8) # Array (H,S,V) for the upper threshold bound of the HSV image
error = np.array([80,50,70],np.uint8) # Array of error widths to create the upper and lower threshold bounds above.
...
def get_image(CompressedImage):
    ...
    # All the images used to find the ball are made global so we can display them during debugging.
    global imgBGR
    global imgHSV
    global imgBLUR
    global mask
...
def Init():
    ...
    # rospy.Subscriber("/usb_cam/image/compressed",CompressedImage, get_image)
